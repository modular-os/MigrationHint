...
//  public:
  ...
  void ExternalDependencyMatcher::handleExternalTypeFuncD(const clang::FunctionDecl *FD,
  ...
  void ExternalDependencyMatcher::handleExternalTypeFD(const clang::RecordDecl *RD,
                            clang::SourceManager &SM,
...
// is passed).
  ...
  void ExternalDependencyMatcher::handleExternalTypeVD(const clang::VarDecl *VD, clang::SourceManager &SM,
                            const clang::LangOptions &LO,
                            int &isInFunctionOldValue) {
#ifdef DEPRECATED
    /*
  ...
  void ExternalDependencyMatcher::handleExternalImplicitCE(const clang::ImplicitCastExpr *ICE,
                                clang::SourceManager &SM) {
  ...
  void ExternalDependencyMatcher::handleExternalCall(const clang::CallExpr *CE, clang::SourceManager &SM) {
    if (auto FD = CE->getDirectCallee()) {
      // output the basic information of the function declaration
      if (!SM.isInMainFile(FD->getLocation()) &&
          SM.isInMainFile(CE->getBeginLoc())) {
