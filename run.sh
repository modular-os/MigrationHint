...
TARGET_SOURCE2=$TARGET_KERNEL/mm/zpool.c
...
pushd $BUILD
    ...
    ninja
    set -x
    ./bin/Indexer ${JSON_PATH} 2>&1 > ${LOG}/`date +%Y%m%d-%H%M%S`-indexer.log
    ./bin/CodeAnalysis ${TARGET_SOURCE1} 2>&1 > ${LOG}/`date +%Y%m%d-%H%M%S`-ca.log
